...
use ic_nosql::{DatabaseManager, traits::{Repository, Model}};
...
thread_local! {
    ...
    static ACCOUNT_REPOSITORY: RefCell<Option<AccountRepositoryNosql>> = RefCell::new(None);
}

#[derive(Clone)]
pub struct AccountRepositoryNosql {
    _marker: (),
}

impl AccountRepositoryNosql {
    pub fn new() -> Self {
        Self { _marker: () }
    }
    ...
    pub fn init() -> Result<(), String> {
        ...
        ACCOUNT_REPOSITORY.with(|repo| {
            *repo.borrow_mut() = Some(AccountRepositoryNosql::new());
        });
    ...
    pub fn global() -> Self {
        ACCOUNT_REPOSITORY.with(|repo| match &*repo.borrow() {
            ...
            None => panic!(
                "AccountRepositoryNosql not initialized! Call AccountRepositoryNosql::init() first."
            ),
        })
    }
    ...
    fn get_database(&self) -> Result<ic_nosql::Database<Account, String>, String> {
        DB_MANAGER.with(|manager| {
            ...
            let db_manager = manager
                .as_ref()
                .ok_or("Database manager not initialized")?;
            
            // Create database with secondary key function for owner queries
            db_manager.get_database("accounts", Some(Box::new(|account: &Account| {
                account.get_secondary_key()
            })))
        })
    }
}

impl IAccountRepository for AccountRepositoryNosql {
...
impl Repository<Account> for AccountRepositoryNosql {
    type Error = String;
    ...
    fn find_by_id(&self, id: &<Account as Model>::PrimaryKey) -> Result<Option<Account>, Self::Error> {
        match self.get(id) {
    ...
    fn delete(&self, _id: &<Account as Model>::PrimaryKey) -> Result<bool, Self::Error> {
        // ic-nosql Database doesn't have a delete method, so we'll return false for now
...
#[cfg(test)]
mod account_repository_nosql_tests {
    ...
    fn setup() -> AccountRepositoryNosql {
        // Initialize the nosql repository
        AccountRepositoryNosql::init().expect("Failed to initialize nosql repository");
        // Create a new repository implementation
        AccountRepositoryNosql::new()
    }
    ...
    fn test_insert_account() {
        ...
        let account = create_test_account("nosql-test-id-1", owner);
    ...
    fn test_get_account() {
        let repo = setup();
        let owner = Principal::from_text("2vxsx-fae").unwrap();
        let account = create_test_account("nosql-test-id-2", owner);
        ...
        // Test retrieval
        let result = repo.get("nosql-test-id-2");
    ...
    fn test_get_nonexistent_account() {
        ...
        let result = repo.get("non-existent-nosql-id");
        assert!(result.is_err(), "Expected error for non-existent account");
        // Note: ic-nosql may have different error messages than the original implementation
    }
    ...
    fn test_exists() {
        let repo = setup();
        let owner = Principal::from_text("2vxsx-fae").unwrap();
        let account = create_test_account("nosql-test-id-3", owner);

        // Insert the account first
        let _ = repo.insert(account).expect("Failed to insert account");

        // Test exists for existing account
        assert!(repo.exists("nosql-test-id-3"), "Expected account to exist");

        // Test exists for non-existing account
        assert!(
            !repo.exists("non-existent-nosql-id"),
    ...
    fn test_find_by_owner() {
        ...
        let accounts_owner1 = vec![
            create_test_account("nosql-test-id-4", owner1),
            create_test_account("nosql-test-id-5", owner1),
            create_test_account("nosql-test-id-6", owner1),
        ];

        // Create and insert an account for a different owner
        let account_owner2 = create_test_account("nosql-test-id-7", owner2);
    ...
    fn test_find_by_nonexistent_owner() {
        ...
        let result = repo.find_by_owner("non-existent-nosql-owner");

        // ic-nosql implementation might return empty list instead of error
        // We'll check for either empty list or error
        match result {
            Ok(accounts) => assert!(accounts.is_empty(), "Expected empty list for non-existent owner"),
    ...
    fn test_update_account() {
        let repo = setup();
        let owner = Principal::from_text("2vxsx-fae").unwrap();
        let mut account = create_test_account("nosql-test-id-8", owner);
        ...
        let retrieved = repo.get("nosql-test-id-8").expect("Failed to get account");
        assert_eq!(
    ...
    #[test]
    fn test_nosql_specific_features() {
        let repo = setup();
        let owner = Principal::from_text("2vxsx-fae").unwrap();

        // Test that accounts are properly stored with ic-nosql backend
        let account1 = create_test_account("nosql-feature-test-1", owner);
        let account2 = create_test_account("nosql-feature-test-2", owner);

        // Insert both accounts
        let _ = repo.insert(account1.clone()).expect("Failed to insert account1");
        let _ = repo.insert(account2.clone()).expect("Failed to insert account2");

        // Verify both accounts can be found by owner using secondary index
        let found_accounts = repo.find_by_owner(&owner.to_string()).expect("Failed to find accounts");
        assert!(found_accounts.len() >= 2, "Should find at least 2 accounts for owner");
